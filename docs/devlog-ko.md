...
- 기본 타이머 및 집중 화면의 세션 토마토 아이콘 크기를 기존 36x36에서 64x64로 대폭 상향하여 시각적 강조성 확보.
- 알림 오버레이 화면 요소의 오프셋 보정으로 프라이팬이 화면 정중앙에 위치하도록 조정.
- `Animated.loop` 기반으로, 알림 무시 에스컬레이션 레벨(Lv.1~Lv.3+)에 따라 프라이팬의 흔들림(shake) 애니메이션 진동과 속도가 극적으로 가속되도록 추가.
