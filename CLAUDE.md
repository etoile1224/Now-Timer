...
- **로드맵 문서**: `docs/feedback-roadmap.md` — 내부 테스터 피드백 기반 Phase 1~6 정의
- **작업 시작 전**: 반드시 해당 Phase의 "작업 항목" 섹션을 먼저 확인하고 범위를 사용자에게 재확인
- **Phase 진행 중**: 코드 변경할 때마다 로드맵의 해당 체크박스를 `[ ]` → `[x]`로 업데이트
- **Phase 완료 시**: 해당 Phase의 "완료 기록" 섹션에 다음을 추가
  - 완료 일자
  - 변경된 주요 파일 목록 (한 줄씩)
  - 핵심 구현 방식 요약 (2-3줄)
  - 커밋 해시 (단축형)
- **새로운 발견**: 작업 중 새로 파악된 이슈/제약은 같은 Phase의 "작업 항목"에 추가하거나 백로그(Phase 6 이후)에 적재
- **Phase 단위 커밋**: 가급적 한 Phase가 하나 이상의 응집된 커밋이 되도록. 작은 수정까지 따로 커밋하지 말 것
