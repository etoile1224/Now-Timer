...
// Expanded 64-color palette for richer color reproduction
const PALETTE = [
  // Grayscale (8)
  ...
  // Extra saturated accents (4) — fills palette gaps
...
/** Weighted Lab distance — heavy chroma weight to keep colors vivid */
function colorDistLab(L: number, a: number, b: number, pi: number): number {
  const [pL, pa, pb] = PALETTE_LAB[pi];
  const dL = L - pL;
  const da = a - pa;
  const db = b - pb;
  // Chroma (color) gets 2x weight vs lightness — preserves hue over brightness
  return dL * dL + 2.0 * da * da + 2.0 * db * db;
}
...
function ditherGrid(
  ...
  for (let y = 0; y < h; y++) {
    ...
    for (let x = 0; x < w; x++) {
      ...
      // Distribute error to neighbors (reduced strength — full F-S is too noisy at 32x32)
      const strength = 0.55; // 55% error diffusion — cleaner pixel art look
      const spread = (dx: number, dy: number, factor: number) => {
        const nx = x + dx, ny = y + dy;
        if (nx >= 0 && nx < w && ny < h) {
          const ni = (ny * w + nx) * 4;
          pixels[ni] += er * factor * strength;
          pixels[ni + 1] += eg * factor * strength;
          pixels[ni + 2] += eb * factor * strength;
        }
      };
      spread(1, 0, 7 / 16);
      spread(-1, 1, 3 / 16);
...
router.post("/convert-photo", async (req: Request, res: Response) => {
  try {
    ...
    // Process at 4x for maximum detail preservation
    const intermediate = await sharp(buffer)
      .resize(GRID_SIZE * 4, GRID_SIZE * 4, { fit: "cover", position: "centre" })
      .modulate({ saturation: 2.2, brightness: 1.05 })
      .linear(1.3, -30) // stronger contrast
      .sharpen({ sigma: 2.0 })
      .toBuffer();
